 ...
 */
public class CruxDateBox extends Composite implements HasEnabled, HasValue<Date>,
	IsEditor<LeafValueEditor<Date>> {
	/**
	 * Default {@link CruxDateBox.CruxFormat} class. The date is first parsed using the
	 ...
	 * the {@link CruxDateBox}. The style will be removed when either a successful
	 * {@link #parse(CruxDateBox,String, boolean)} is called or
	 * {@link #format(CruxDateBox,Date)} is called.
	 * <p>
	 * Use a different {@link CruxDateBox.Format} instance to change that behavior.
	 * </p>
	 */
	public static class DefaultFormat implements CruxFormat {
		...
		public String format(CruxDateBox box, Date date) {
			if (date == null) {
				return "";
			} else {
		...
		@SuppressWarnings("deprecation")
		public Date parse(CruxDateBox dateBox, String dateText, boolean reportError) {
			Date date = null;
			try {
		...
		public void reset(CruxDateBox dateBox, boolean abandon) {
	 ...
	 */
	public CruxDateBox(DatePicker picker, Date date, CruxFormat format) {
		this.picker = picker;
	 ...
	 * {@link CruxDateBox}. If this {@link CruxDateBox} is not empty, the contents of date
