...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template name="scrollBanner" match="adv:scrollBanner">
		<xsl:element name="span" namespace="{$xhtmlNS}">
			<xsl:call-template name="widgetAttributes"/>
			<xsl:if test="string-length(@messageScrollingPeriod) > 0">
				<xsl:attribute name="_messageScrollingPeriod" select="@messageScrollingPeriod"/>
			</xsl:if>		
