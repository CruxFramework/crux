...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template name="hyperlink" match="crux:hyperlink">
	
		<xsl:element name="span" namespace="{$xhtmlNS}">

			<xsl:call-template name="widgetAttributes"/>
			
			<xsl:if test="string-length(@targetHistoryToken) > 0">
				<xsl:attribute name="_targetHistoryToken" select="@targetHistoryToken"/>
			</xsl:if>
