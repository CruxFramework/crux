...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template name="splitPanel">
	
		<xsl:element name="span" namespace="{$xhtmlNS}">

			<xsl:call-template name="widgetAttributes"/>
			
			<xsl:if test="string-length(@splitPosition) > 0">
				<xsl:attribute name="_splitPosition" select="@splitPosition"/>
			</xsl:if>
			
			<xsl:for-each select="child::*">
				<xsl:element name="span" namespace="{$xhtmlNS}">
					<xsl:attribute name="_position" select="local-name()"/>
