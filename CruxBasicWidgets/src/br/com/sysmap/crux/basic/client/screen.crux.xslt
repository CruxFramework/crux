...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template name="screen" match="crux:screen">
		...
		<xsl:element name="span" namespace="{$xhtmlNS}">
			...
			<xsl:if test="string-length(@manageHistory) > 0">
				<xsl:attribute name="_manageHistory" select="@manageHistory"/>
			</xsl:if>
			...
			<xsl:if test="string-length(@useSerializable) > 0">
				<xsl:attribute name="_useserializable" select="@useSerializable"/>
			</xsl:if>

			<xsl:if test="string-length(@useController) > 0">
