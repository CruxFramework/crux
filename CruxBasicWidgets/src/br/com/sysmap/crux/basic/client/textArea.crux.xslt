...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template name="textArea" match="crux:textArea">
		<xsl:element name="span" namespace="{$xhtmlNS}">
			<xsl:call-template name="widgetAttributes"/>
			<xsl:if test="string-length(@characterWidth) > 0">
				<xsl:attribute name="_characterWidth" select="@characterWidth"/>	
			</xsl:if>
			<xsl:if test="string-length(@visibleLines) > 0">
				<xsl:attribute name="_visibleLines" select="@visibleLines"/>	
			</xsl:if>
