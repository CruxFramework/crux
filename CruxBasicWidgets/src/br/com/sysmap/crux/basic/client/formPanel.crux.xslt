...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template name="formPanel" match="crux:formPanel">
		...
		<xsl:element name="span" namespace="{$xhtmlNS}">
			...
			<xsl:if test="string-length(@method) > 0">
				<xsl:attribute name="_method" select="@method"/>
			</xsl:if>
			
			<xsl:if test="string-length(@action) > 0">
				<xsl:attribute name="_action" select="@action"/>
			</xsl:if>
			
			<xsl:if test="string-length(@encoding) > 0">
				<xsl:attribute name="_encoding" select="@encoding"/>
			</xsl:if>
