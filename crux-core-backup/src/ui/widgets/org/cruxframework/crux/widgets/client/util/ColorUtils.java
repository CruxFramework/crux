	...
	public static int[] getRGB(String color)
	{
		try
		{
			...
			if (color.startsWith("rgb"))
			{
				...
				index = color.indexOf(',');
				r = color.substring(0, index).trim();
				color = color.substring(index+1);
				index = color.indexOf(',');
				g = color.substring(0, index).trim();
				color = color.substring(index+1);
				index = color.indexOf(',');
				if (index < 0)
				{
				...
				b = color.substring(0, index).trim();
				return new int[] { Integer.parseInt(r), Integer.parseInt(g), Integer.parseInt(b) };
			}
			else
			{
				if (color.startsWith("#"))
				...
				return new int[] { Integer.parseInt(r, 16), Integer.parseInt(g, 16), Integer.parseInt(b, 16) };
			}
		}
		catch (Exception e)
		{
